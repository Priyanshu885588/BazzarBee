...
const addToCart = async (req,res) => {
    ...
    try {
      ...
      if(userCart.length === 0){
        ...
        return res.status(200).json({cart:updatedUserCart,msg:"product added to cart successfully"})
      }
      else{
        ...
        items.forEach((newItem) => {
          const existingItemIndex = updatedItems.findIndex(item => item.productId.equals(newItem.productId));
          if (existingItemIndex >= 0) {
            // If the item already exists in the cart, update the quantity and total
            updatedItems[existingItemIndex].quantity += newItem.quantity;
            updatedItems[existingItemIndex].total += newItem.quantity * newItem.price;
          } else {
        ...
        return res.status(200).json({ cart: updatedCart, msg: "Cart updated successfully" });
      }
    } catch (error) {
...
const removeCart = async (req,res) =>{
    ...
    try {
        ...
        const subTotal = userCart.items.reduce((acc, item) => acc + (item.quantity * item.price), 0);
...
const getAllCartProducts = async (req,res) => {
    ...
    try {
        ...
        res.status(200).json({cartItems:items,msg:"Products found successfully"})
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
}
